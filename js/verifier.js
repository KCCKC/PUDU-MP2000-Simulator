...
window.evaluateModel = function(modelCode, pallet) {
  ...
  const L = Number(pallet.dimensions?.length ?? 1200);
  const W = Number(pallet.dimensions?.width ?? 1000);

  // Turning Radius & Aisle Width Calculations (Official PUDU MP2000 Manual)
  let rMin = 2.00;
  let astMin = 2000;
  if (L === 1200 && W === 1000) {
    rMin = 2.00;
    ...
    rMin = Math.max(1.48, Math.round(Math.sqrt(pivotToFront * pivotToFront + halfWidth * halfWidth) * 1.043 * 100) / 100);
    astMin = Math.round(rMin * 1000);
  }
  const rRec = Math.round((rMin + 0.20) * 100) / 100;
  const astRec = astMin + 400;
